...
export class Temperature extends BaseChart {
  static updateChart(data, dateIni, dateEnd, context) {
    ...
    let tMaxMin = {
      tmax: filteredData.maxTemperature,
      tmin: filteredData.minTemperature,
    };
  ...
  static getTSecoData(actions) {
    let data = this.getDataHandler().getSavedData(
      process.env.MIX_DATA_INTERVAL_STORAGE
    );
