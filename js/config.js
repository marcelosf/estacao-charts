const DATA_INTERVAL_STORAGE = 'dataInterval';
const PRESSURE_STORAGE = 'pressure';
const WIND_STORAGE = 'wind';
const PRECIPITATION_STORAGE = 'precipitation';

const API = 'consulta/charts/loadtseco.php';

const PRESSURE_INI_DATE_FIELD = '#pressure-ini-date-field';
const PRESSURE_END_DATE_FIELD = '#pressure-end-date-field';

const WIND_INI_DATE_FIELD = '#wind-ini-date-field';
const WIND_END_DATE_FIELD = '#wind-end-date-field';

const PRECIPITATION_INI_DATE_FIELD = '#precipitation-ini-date-field';
const PRECIPITATION_END_DATE_FIELD = '#precipitation-end-date-field';


const IMAGES = "http://10.60.0.57/consulta/charts/images/";